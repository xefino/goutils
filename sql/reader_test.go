...
// modules in the sql package
func TestSQL(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "SQL Suite")
}

var _ = Describe("SQL Read - Tests", func() {

	// Tests the conditions under which the ReadRowsAsync function will return an error
	DescribeTable("ReadRowsAsync - Failures",
		func(extraColumn bool, dataInvalid bool, rowsFailed bool, message string) {
			...
			objChan := make(chan *testValue, 1)
			errChan := make(chan error, 1)
			ReadRowsAsync(rows, objChan, errChan)
			err = <-errChan
	...
	// Tests that, if the ReadRowsAsync function does not fail, then all the rows returned from the
	// query will be sent to the object channel and the error channel will contain an EOF error to
	// signal that the operation is compplete
	It("ReadRowsAsync - No failures - Data streamed", func() {
		...
		// Finally, attempt to read the data received from the query; this should not fail
		objChan := make(chan *testValue2, 1)
		errChan := make(chan error, 1)
		ReadRowsAsync(rows, objChan, errChan)

		// Verify that the error retrieved from the channel is an EOF
		err = <-errChan
		Expect(err).Should(HaveOccurred())
		Expect(err).Should(Equal(EOF))
		Expect(mock.ExpectationsWereMet()).ShouldNot(HaveOccurred())

		// Verify the data
		data := <-objChan
		Expect(data.private).Should(BeEmpty())
		Expect(data.Key1).Should(Equal(420))
		Expect(data.Key2).Should(Equal(69))
		Expect(data.Value1).Should(Equal("derp"))
		Expect(data.Value2).Should(Equal("herp"))
		Expect(data.Value3).Should(Equal(float32(11.899999618530273)))
		Expect(data.Value4).Should(BeTrue())
		Expect(data.Defined).Should(Equal(defined([]byte("32"))))
	})
	...
	// Tests that, if the ReadRows function does not return an error, then all the rows returned from
	// the query will be read into a list of data objects and returned
